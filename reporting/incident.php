...
<style>
    ...
    <a class="submit print" target="_blank" href="print.php?id=<?=$_GET['id'];?>">Print</a>
    <a class="submit print" download="incident.xls" href="xls_incident.php?id=<?=$_GET['id'];?>">Export to Excel</a>
    <a class="submit print" download="incident.pdf" href="pdf_incident.php?id=<?=$_GET['id'];?>">Export to PDF</a>
...
<table id="incidentTable" class="table table-bordered">
    ...
    <tr>
        ...
        <td colspan="2">
            ...
            <a data-toggle="modal" data-target="#reportMap"><?=$incident['map_coordinates'];?></a>
            <?
            $coords = explode(",", $incident['map_coordinates']);
            ?>
            <input type="hidden" id="latmap" value="<?=$coords[0];?>">
            <input type="hidden" id="lngmap" value="<?=$coords[1];?>">

            <!-- Modal -->
            <div class="modal fade" id="reportMap" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
        ...
        <td colspan="7">
            ...
            if (count($evidenceFiles) > 0) {
                foreach ($evidenceFiles as $file) {
                    ?>
                    <a download href="../evidence/<?=$file['id']."_".$file['file_name'];?>"><?=$file['file_name'];?></a><br>
                <?
                }
            } else {
                ?>No files added<?
    ...
    if ($incident['entonox'] == "1") {
        ...
        <tr>
            ...
            <td colspan="3">
                ...
                <img src="data:image/jpeg;base64,<?php echo $incident['ws'];?>"/>
            </td>
            <td colspan="3">
                <?=$usersList[$incident['witness_sec_id']]['name'];?> <?=$usersList[$incident['witness_sec_id']]['surname'];?>
                <img src="data:image/jpeg;base64,<?php echo $incident['wss'];?>"/>
            </td>
        </tr>
    <?
    }
    ...
    <tr>
        ...
        if ($incident['dr_id'] >0) {
            ?>
            <td colspan="3">
                <img src="data:image/jpeg;base64,<?php echo $incident['drs'];?>"/>
            </td>
        <?
        }
        ?>
        ...
        ?>">
            <img src="data:image/jpeg;base64,<?php echo $incident['ps'];?>"/>
        </td>
        <td colspan="<?
        if ($incident['dr_id'] >0) {
            ?>3<?
        ...
        ?>">
            ...
            if ($incident['unable_to_sign'] == 1) {
                ...
                ?>
                <img src="data:image/jpeg;base64,<?php echo $incident['cs'];?>"/>
            <?
            }
            ?>
        </td>
